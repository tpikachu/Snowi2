...
async function connectSession(streaming, socket) {
...
const audioPayloads = (socket) =>
  socket.sent
    ...
    .filter((msg) => msg.type === "input_audio_buffer.append")
    .map((msg) => msg.audio);
...
test("connect() preserves audio buffered during beginConnecting() instead of wiping it", async () => {
  ...
  const payloads = audioPayloads(streaming.ws);
  assert.deepEqual(payloads, [
...
test("close after session_expired does not fire onSessionEnd (reconnect owns the session)", async () => {
  ...
  const streaming = new OpenAIRealtimeStreaming();
  const socket = makeFakeSocket(WS.CONNECTING);
  await connectSession(streaming, socket);
