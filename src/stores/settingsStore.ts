...
function migrateProviderSettings() {
  ...
  // Legacy non-BYOK (managed cloud) installs land on "local" — the cloud
  // pipeline no longer exists.
  const transcriptionMode =
    useLocal || cloudMode === "byok" ? deriveTranscriptionMode(useLocal, provider) : "local";
...
// so nothing is copied and the upload context falls through to its defaults.
...
// The managed "openwhispr" cloud mode was removed along with cloud accounts.
// Normalize any persisted selection of it to local processing (the vision
// override, which never had a local lane, falls back to BYOK providers), and
// collapse the cloud/BYOK routing axis to "byok". Idempotent, so it needs no
// completion marker. Runs after the legacy migrations above, which are the
...
const OPENWHISPR_TRANSCRIPTION_MODE_KEYS: ReadonlyArray<[string, string]> = [
...
const OPENWHISPR_CLOUD_MODE_KEYS = [
  ...
  "dictationAgentVisionCloudMode",
] as const;

function migrateRemovedOpenWhisprMode() {
  if (!isBrowser) return;
  for (const key of OPENWHISPR_LLM_MODE_KEYS) {
    if (localStorage.getItem(key) === "openwhispr") localStorage.setItem(key, "local");
  }
  if (localStorage.getItem("dictationAgentVisionMode") === "openwhispr") {
    localStorage.setItem("dictationAgentVisionMode", "providers");
  }
  for (const [modeKey, useLocalKey] of OPENWHISPR_TRANSCRIPTION_MODE_KEYS) {
    if (localStorage.getItem(modeKey) === "openwhispr") {
      localStorage.setItem(modeKey, "local");
      localStorage.setItem(useLocalKey, "true");
    }
  }
  for (const key of OPENWHISPR_CLOUD_MODE_KEYS) {
    if (localStorage.getItem(key) === "openwhispr") localStorage.setItem(key, "byok");
  }
  // Vestige of the removed cloud-account session state.
...
export const useSettingsStore = create<SettingsState>()((set, get) => ({
  ...
  // Snowy is dark-first: a new user lands in dark mode. "light" and "auto"
  ...
  chatAgentModel: readString("chatAgentModel", "gpt-5-mini"),
  ...
  mirrorTranscriptionToDerivedScopes: () => {
    ...
    // scope — otherwise the UI keeps showing the previous mode even though the
    // cloud routing now points at the new provider.
    const mode = deriveTranscriptionMode(useLocalWhisper, cloudTranscriptionProvider);
      ...
      if (!localStorage.getItem("enterpriseSetupMode")) {
        ...
        const hasChosenProvider =
          Object.values(INFERENCE_SCOPES).some((scope) =>
            Boolean(localStorage.getItem(scope.storeKeys.mode as string))
          ) ||
          Boolean(
