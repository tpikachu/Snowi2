...
const FAST_ANSWER_SYSTEM_PROMPT = [
  ...
  "",
  "Format for a glance, not a read: bold the decisive words — the number, the",
  "date, the name — with **…**. A one-sentence answer stays one sentence. When",
  "there is genuinely more to unpack, shape it like a briefing card: the direct",
  "answer first, then a short bold label ending in a colon (like **Current",
  "point:**) over a dash list of at most three items, each opening with its",
  "bolded key phrase — and, only when it earns its line, one closing takeaway.",
  "When handing the user a line to say out loud, put it on its own line wrapped",
  "in backticks (`…`) so it stands apart from advice about it. No headings, no",
  "tables.",
  "",
