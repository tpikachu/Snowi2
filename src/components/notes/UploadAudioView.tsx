...
const BYOK_MAX_FILE_SIZE = 25 * 1024 * 1024; // 25 MB — provider hard limit for bring-your-own-key
...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  const effectiveCleanupModel = useSettingsStore((settings) => settings.cleanupModel);
  ...
  // Local / self-hosted / custom endpoints: no limits
  // BYOK: 25 MB provider hard max
  const byokTooLarge =
    !!file &&
    isByok &&
    !isSelfHosted &&
    cloudTranscriptionProvider !== "custom" &&
    file.sizeBytes > BYOK_MAX_FILE_SIZE;
  ...
  const cancelTranscription = () => {
    // BYOK providers can't be aborted mid-flight; the run-id bump discards any
    // late result instead.
    runIdRef.current++;
    reset();
  };
...
function SelectedView({
  ...
  return (
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      {/* BYOK file too large — provider hard limit */}
      {byokTooLarge && (
