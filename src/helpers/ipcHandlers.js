...
// Uniform IPC failure shape for BYOK/streaming handlers: keeps `code` (and any
// caller-attached status) so the renderer can branch on the error kind.
  ...
  setupHandlers() {
    ...
    ipcMain.handle("bedrock-list-models", async (event, config) => {
      ...
      try {
        ...
        let credentials;
        if (runtime.enterprise.bedrockProfile) {
    ...
    ipcMain.handle("capture-screen-context", async () => {
      ...
      // A failed rect read falls back to the cursor rather than rejecting:
      // capture's contract is to yield null, never to throw — a screenshot must
      // never break the dictation it accompanies.
    ...
    // Manual (BYOK) enterprise runtime. The managed-credential broker that used
    // to resolve workspace-issued Bedrock/Azure identities was removed with
    // cloud accounts; every value now comes from the renderer's stored config.
    ...
    const fetchRealtimeToken = async (_event, options, { streams } = {}) => {
