...
- **ReasoningService.ts**: AI processing for agent-addressed commands
  ...
  - **Models are picked at point of use, not in Settings** (client direction, 2026-09): `ModelPickerChip` (`src/components/ModelPickerChip.tsx`, options built by pure `src/utils/modelPickerOptions.ts` — keyed cloud providers first, downloaded local models, then keyless providers as "Add key" rows deep-linking to Settings; every model row carries the registry's localized one-liner via `descriptionKey`; OpenRouter's group is the curated slice in `src/config/openrouterModels.ts` — registry models under their OpenRouter slugs, labels borrowed from the vendor entry, slugs verified against openrouter.ai — plus an "Other model…" row that takes any id typed in, `acceptsAnyModelId` on the group) sits in the chat composer and the meeting cue card (both write `chatIntelligence` — one shared pick), in each action's editor and the follow-up email dialog (per-item overrides). **Entering a key IS the setup**: the BYOK key setters route through `createProviderKeySetter`, and on a provider key's empty→set transition `applyDefaultModelsForNewKey` assigns that provider's per-scope defaults (`src/utils/scopeModelDefaults.ts`, pure + unit-tested — e.g. OpenAI → chat `gpt-5-mini`, actions `gpt-5-nano`) to any scope that isn't already ready; ready scopes and enterprise-managed scopes are never touched, so a later key or an explicit pick is never overridden by a key alone. **The provider card is the switch**: `setCoreCloudProvider(providerId)` routes chat AND write-ups at the provider — keeping a model already picked on it, else its scope defaults — and the grid's highlighted card is derived from the store (chat's provider, else write-ups'), wearing an "In use" badge; a click on a card only SELECTS it (its key, its badges — since 2026-09-22, when two providers serving made a click-that-switches unable to show the other key without rerouting chat), a card without a key holds as a pending choice until its FIRST key is saved (that save is the switch), a keyed card that does not serve both scopes offers the explicit "Run chat and write-ups on X" button (`useForBoth` → `setCoreCloudProvider`), and the hint under the key field says which of those is happening. Before 2026-09-15 the cards only chose which key box was shown, `setOpenrouterApiKey` was a plain secret setter, and OpenRouter was filtered out of the chip — so a client who added an OpenRouter key found the write-ups still on OpenAI with no way to move them (behavior test: `test/helpers/scopeDefaultsOnKey.test.js`). **Several providers serve at once, and the page says so** (client discussion, 2026-09-22 — chat on OpenRouter's Claude, write-ups on OpenAI's GPT is a supported setup: every request resolves its own scope's provider, endpoint and key): each card wears a badge per scope it serves ("Chat", "Write-ups"; `ProviderGrid`'s `roles`, `[data-provider-roles]`) in place of the former single "In use", and the hint under the key field names what the selected card serves (`cloudInUse` for both, `cloudServesChat` / `cloudServesWriteups` for one). **A removed key moves what ran on it**: `createProviderKeySetter` on the set→empty transition calls `rerouteScopesOffProvider` — pure `utils/providerReroute.ts` plans it: every `providers`-mode scope on that provider goes to the next keyed provider's scope default (`firstKeyedDefaultableProvider`, the removed one already keyless), or with none left is cleared to "needs a model" (Home's capabilities card and the dot's badge show it; write-ups fail with the noModel label and its Configure remedy instead of "<provider> API key not configured") — and the moves are queued on `routeNoticeStore` for `RouteNoticeToastListener` (mounted beside the background-action listener), one card per scope naming where it went and why. Re-entering the key never overrides a working scope — the chip is where a person picks that model again. A key removed while a meeting is recording through that provider's SPEECH route cannot move anything: `LanguageModelsPanel` warns (`reroute.speechInUse`) that the session keeps its token and the next meeting needs a key, and the transcription error toast is already one card per fault. OpenRouter has a fast-lane sibling (`FAST_LANE_MODELS.openrouter`), so a Claude chat there still gets a fast lane. E2E: the second provider test in `e2e/settings.spec.js`. Settings → Language Models is ONE page (`LanguageModelsPanel.tsx`, tab id `providers` — the former `actions`/`chatIntelligence` tabs are deleted from `LlmTab`, and the settings nav renders no sub-items unless a section has >1 visible panel): an **engine choice** (Cloud Providers | Local cards, `InferenceModeSelector`) applied to both scopes at once via `setCoreLlmEngine` in settingsStore (radio semantics — flipping to cloud seeds the first keyed provider's scope defaults into any unready scope; flipping to local routes both at the shared local model; enterprise-managed scopes untouched). Cloud shows a `ProviderGrid` card per provider (dot = key saved, badge = in use) over one key field; Local shows the shared GGUF picker (writes both scopes). **Local language models are on offer, labelled** (`LOCAL_LLM_ENABLED = true` in `src/config/features.js`; hidden 2026-09-15 after a side-by-side against the reference product where the thin answer had come from Qwen3.5 9B at 4-bit with nothing on the row saying what it was good for, back 2026-09-18 on the condition that every local row answers three questions at the moment of choice). Pure `utils/localModelLabels.ts` computes them: a **tier** (best local answers / fast / quick tasks — the registry's curated `tier`, else by parameter count read off the id, Gemma's `e4b` prefix and MoE totals included), the **memory** it needs (weights × 1.15 + 1 GB, rounded up) against the cached hardware probe (`get-capability-snapshot` IPC → `describeLocalModel` fit: good / tight / poor, a discrete GPU's VRAM counting; poor rows go dim with a warning line), and **what it gives up**: web search on every local row, notes search too below 4B (`localModelCanUseTools` — the same predicate `useChatStreaming` strips the tools with). The chip's local group and Settings → Local both lead with one honest line (`models.local.note`: private and free, slower and shorter than cloud, no web search). What a row gives up is a **warning badge** on its helper line, after the memory figure — on the title row, beside name, tier and Recommended, it squeezed the name into an ellipsis at ordinary widths — (`ui/LocalModelCaveat.tsx`, in the chip and on the Settings card alike; `[data-local-caveat]` = `web` | `tools`): amber, one notch softer than the memory warning because a limitation may be chosen on purpose for privacy — and its tooltip (the `Tooltip`'s new `wrap` plate) explains the feature, not the badge: what web search does during a meeting and where such a model answers from instead (`models.local.webSearchHint` / `webAndNotesSearchHint`; client direction 2026-09-22 — as a muted phrase glued to the memory figure it had read as a spec line). The memory figure stays the helper line. In Settings the current generation leads — `featured: true` in the registry, fourteen models across the six families — and the rest sit under a "Show N more models" row (`splitFeaturedModels`; anything on disk or in use is never tucked away). Cloud stays the recommended path: a fresh install defaults both scopes to `providers` mode (until 2026-09-18 the default was `local` holding OpenAI's id), and `normalizeLocalMode` in settingsStore resolves such a cloud id under local mode as cloud on the READ path — with the flag off it retires every local scope as cloud (the stored cloud provider when its key is present, else the first keyed provider's scope defaults, else the stored cloud provider unkeyed or, for a local family id, "needs setup"), a path that stays unit-tested through the function's `enabled` parameter (`test/helpers/localLlmMode.test.js`). Nothing is deleted; local SPEECH models are unaffected. That's the WHOLE page: the former "Advanced setup" disclosure (per-scope `InferenceConfigEditor`s, `ChatAgentSettings` with the fast-lane override + chat prompt, Manage actions + auto-title) was removed on client direction 2026-09 — `ChatAgentSettings.tsx` is deleted, the fast lane always auto-derives (its `useChatFastModel`/`chatFast` store keys persist but have no UI), LAN/custom/enterprise chat-and-actions setups have no UI either (existing stored configs still resolve), actions are managed only from the notes sidebar, and the auto-title toggle moved to Preferences → Notes (`general` section, anchor `notes`). The `configureActions`/`configureChatIntelligence` remedies land on this page, and the Home capabilities card shows two rows (transcription + AI model). **A cloud speech provider is only as ready as its key** (client, 2026-09-22: Speech-to-Text on Cloud Providers with no key wore "Active" on the engine card and on the chosen model, and Home called transcription working, so the first meeting failed with "No OpenAI API key configured"): pure `utils/speechRouteReady.ts` judges the meeting route the way the request path will — a provider the meeting route can stream through, every secret in `PROVIDER_CREDENTIALS` stored (Corti needs both halves; xAI and Mistral were missing from the bar's old check), and a model picked — and `selectMeetingSpeechReadiness` in settingsStore is the one answer Home's transcription row (`ready`, with an `items.transcription.missing` line and the `descriptionSetup` lead copy), the dot's setup warning (`selectSpeechOk` in `useBarStatusPublisher`) and the Speech-to-Text page read. There the Cloud Providers card wears an amber `status` badge (`InferenceModeOption.status`, `[data-mode-status]`: "Needs key" / "Pick a model") in place of "Active", and the chosen model row the same through `ModelCardList`'s `selectedBadge` (`[data-selected-badge="warning"]`). Local engines stay "ready" here — a missing download is the dot's own gate. E2E: the last test in `e2e/settings.spec.js`. Per-action overrides live as `model_mode/model_provider/model_id` columns on the `actions` table (null = default), read via pure `src/utils/actionModelOverride.ts` — a stale override (deleted key, removed local model) silently degrades to the actions default; follow-up email stores its override in `followUpModelMode/Provider/Id` settings keys. `ModelPickerChip` takes `defaultScope` so an override picker's "App default" row names the model it resolves to ("App default (GPT-5 Nano)")
