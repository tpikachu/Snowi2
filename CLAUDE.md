...
A meeting note can be recorded in several sessions — "Resume meeting" in the note's bottom bar (the record slot of `NoteBottomBar`, labeled via its `resumeLabel` prop; the same slot becomes the elapsed/stop control while recording) restarts capture into the same note. The plumbing predates the button: `startRecording({ noteId, seedSegments })` in `meetingRecordingStore.ts` is called by `PersonalNotesView.startRecording` with the note's parsed transcript as seed, so new finals append (timestamp-sorted) to the existing array and the periodic autosave writes the merged transcript back. Session bookkeeping rides `seedSegmentCountValue` (module flag, mirrored into `pendingStop.seedSegmentCount`):
