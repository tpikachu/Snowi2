...
- The note composer (`NoteBottomBar`) has no idle mic anymore (`canRecord={isRecording}`): recordings start from Start meeting / detection / Resume, and the bar shows only the elapsed/stop control while one runs. Keeping a written-up note in sync is unchanged: new segments mark the write-up stale and Generate Notes regenerates over the whole merged transcript
