...
1. **Dual Window Architecture**:
   ...
   **Assistant bar + cue card** (`?agent=true`): one persistent floating window is the product's daily face. Created at startup and shown per `barStartupPolicy.js` (pure, unit-tested; the control panel only appears at launch for onboarding — otherwise it is created **hidden** via `createControlPanelWindow({ background: true })`, because its renderer owns the capture graph and the panel bridge). Idle, the window is a 104px two-row bar (ask field over a control strip). Clicking/focusing the ask field opens the **command palette** below it (window grows to `PALETTE_HEIGHT`): actions (Start meeting, window toggle) plus every Settings section from `SETTINGS_SECTIONS`; typing filters rows (`src/utils/barPalette.ts`, pure + unit-tested), Enter with text still asks the agent, arrows navigate, Escape peels palette-then-window; settings rows deep-link via `open-control-panel` with `{ settings: { section, panel } }` → `open-settings-section` event → ControlPanel opens SettingsModal there (the agent's `open_settings` tool uses the same path). The chat agent also carries app-control tools (`src/services/tools/appControlTools.ts`): `get_app_settings` (curated snapshot via pure `appSettingsSnapshot.ts`, never credentials), `set_hotkey` (same store setters/IPC as the Settings UI; dictation trio gated by `DICTATION_ENABLED`), `open_settings`. Other bar furniture: Start meeting (via `start-manual-meeting`), a compact pulsing warning icon for missing setup (`useBarSetupStatus`, fed per-scope — speech/actions/chat — by `useBarStatusPublisher` in the control panel renderer; hover lists the gaps, click = `open-control-panel` with target `"setup"` → Home with the capabilities card forced open via `homeSetupStore`), a window toggle (`toggle-control-panel` IPC), and Escape/X to hide (a tray-icon click or the tray menu's "Show assistant bar" brings it back — the tray click summons the bar, not the control panel). Speech-model download progress rides the same bar-status channel (progress events only reach the control panel window, which started the download): a blocking download turns Start meeting into "Downloading… N%", a non-blocking one shows a quiet percent pill. E2E coverage: `e2e/bar.spec.js` (`npm run test:e2e`). The window is visible in screen shares by default; the `overlayStealth` setting (Settings → Startup, persisted as `OVERLAY_STEALTH`) opts into content protection for both bar and cue card. While a meeting records the same window renders `MeetingPanelOverlay` — the cue card — morphing in place via `resizeAgentWindow`. The capture graph lives in the control panel's renderer, so the card is a _view_: `useMeetingPanelBridge` publishes `MeetingPanelSnapshot`s (`src/utils/meetingPanelSnapshot.ts`, pure + unit-tested) to main (`updateMeetingPanel`, edge-triggered on `isRecording`), which forwards state to the bar window; the card's buttons send allow-listed commands (`pause`/`resume`/`stop`/`open`/`configureModels`) back through main to the same store functions the in-app controls call. Mic level rides a separate throttled channel. When the meeting ends, main surfaces the control panel — the keep-or-discard prompt and the AI write-up land there.
...
- **ReasoningService.ts**: AI processing for agent-addressed commands
  ...
  - **Models are picked at point of use, not in Settings** (client direction, 2026-09): `ModelPickerChip` (`src/components/ModelPickerChip.tsx`, options built by pure `src/utils/modelPickerOptions.ts` — keyed cloud providers first, downloaded local models, then keyless providers as "Add key" rows deep-linking to Settings) sits in the chat composer and the meeting cue card (both write `chatIntelligence` — one shared pick), in each action's editor and the follow-up email dialog (per-item overrides). Settings → Language Models leads with a **providers panel** (`ProviderKeysPanel.tsx`, tab id `providers`, first `LlmTab`) holding only the 7 BYOK API keys; the old per-scope `InferenceConfigEditor`s survive behind `ScopeModelSummary`'s "Advanced setup" disclosure (LAN/custom/enterprise/local downloads). Per-action overrides live as `model_mode/model_provider/model_id` columns on the `actions` table (null = default), read via pure `src/utils/actionModelOverride.ts` — a stale override (deleted key, removed local model) silently degrades to the actions default; follow-up email stores its override in `followUpModelMode/Provider/Id` settings keys
